...
async function createPayment({ amount, currency, reference }) {
  ...
  // No real redirect_url -- there's no real hosted checkout page behind
  // this. A previous version pointed callers at a fake
  // pay.optimise-payments-sandbox.invalid URL, which looked like a real
  // handoff but would just fail to resolve if a caller actually redirected
  // a buyer there. `simulated: true` lets callers (see checkoutService.js)
  // tell buyers honestly that payments aren't connected yet, instead of
